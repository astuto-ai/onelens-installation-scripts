...
if [ "$deployment_type" = "job" ]; then
  ...
  if [ "$patching_mode" = "healthcheck" ]; then
    ...
    if [ -z "$UNHEALTHY_REASONS" ]; then
        ...
        # --- GPU: DCGM exporter lifecycle (only when gpu.enabled=true) ---
        # GPU monitoring is opt-in. Read gpu.enabled from Helm values to decide
        # whether to deploy/maintain DCGM or clean up a previously auto-deployed instance.
        # Non-fatal — failures don't affect healthcheck result.
        _gpu_enabled=$(helm get values onelens-agent -n onelens-agent -o json 2>/dev/null \
            | jq -r '.["onelens-agent"].gpu.enabled // "false"' 2>/dev/null || echo "false")
        if [ "$_gpu_enabled" = "true" ]; then
