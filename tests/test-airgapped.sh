...
# + 1 conditional VictoriaMetrics image override + 2 conditional node-exporter overrides = 14 total --set flags.
# DCGM image is NOT in the helm air-gapped section — it's handled separately via kubectl apply.
# Components using {registry}/{repository}:{tag} (KSM, OpenCost, kube-rbac-proxy, node-exporter)
